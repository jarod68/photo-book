...
PLAY_DOMAIN="play.holtz.fr"
ACME_EMAIL="noname@book.holtz.fr"
PHOTOS_DIR=""
IMAGE="jarod68/photo-book:latest"
MINESIM_IMAGE="jarod68/mine-sim:latest"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      ...
      echo ""
      echo "Options:"
      echo "  --domain       photo-book hostname (default: $DOMAIN)"
      echo "  --play-domain  mine-sim hostname (default: $PLAY_DOMAIN)"
...
# ── Write the mine-sim startup script ─────────────────────────────────────────
# A standalone helper that pulls and (re)starts just the mine-sim service of the
# compose stack (served on $PLAY_DOMAIN, behind the shared Traefik).
write_minesim_script() {
  info "Writing $INSTALL_DIR/start-mine-sim.sh …"
  cat > "$INSTALL_DIR/start-mine-sim.sh" <<MINESIM
#!/usr/bin/env bash
# start-mine-sim.sh — pull and (re)start the mine-sim service (${PLAY_DOMAIN}).
# Generated by deploy.sh.
set -euo pipefail
cd "${INSTALL_DIR}"
echo "[mine-sim] pulling ${MINESIM_IMAGE} …"
${DC} pull mine-sim
echo "[mine-sim] starting …"
${DC} up -d mine-sim
echo "[mine-sim] up → https://${PLAY_DOMAIN}"
MINESIM
  chmod +x "$INSTALL_DIR/start-mine-sim.sh"
  success "start-mine-sim.sh written."
}
...
echo -e "${BOLD}Domain (photo-book)    : ${CYAN}https://${DOMAIN}${RESET}"
echo -e "${BOLD}Domain (mine-sim)      : ${CYAN}https://${PLAY_DOMAIN}${RESET}"
echo -e "${BOLD}ACME e-mail            : ${CYAN}$ACME_EMAIL${RESET}"
echo -e "${BOLD}App image              : ${CYAN}$IMAGE${RESET}"
echo -e "${BOLD}Mine-sim image         : ${CYAN}$MINESIM_IMAGE${RESET}"
...
echo -e "${BOLD}${GREEN}╚══════════════════════════════════════════════════╝${RESET}"
echo
echo -e "  photo-book URL : ${BOLD}https://${DOMAIN}${RESET}"
echo -e "  mine-sim URL   : ${BOLD}https://${PLAY_DOMAIN}${RESET}"
...
echo -e "  Restart mine-sim: ${BOLD}${INSTALL_DIR}/start-mine-sim.sh${RESET}"
echo -e "  Update         : ${BOLD}sudo $0 --update${RESET}"
echo
echo -e "  ${YELLOW}Note:${RESET} DNS ${BOLD}${DOMAIN}${RESET} and ${BOLD}${PLAY_DOMAIN}${RESET} must point to this server's IP address."
echo
