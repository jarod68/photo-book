 ...
 *             + light Gaussian blur on edges for a clean result.
 * Tracking  : <PHOTOS_DIR>/.faces-blurred.json — already-processed files
 *             are never re-analysed, even after a restart.
 * Originals : modified in place (via .tmp file for atomicity).
 *             Corresponding previews are deleted so they get
 *             regenerated by ensurePreview() on next access.
 */
...
async function blurFaces(imagePath, faces) {
  ...
  const composites = (
    await Promise.all(faces.map(async face => {
      ...
      // Pixelation: scale down to ~8% of size then back up
      ...
      const blurred = await sharp(imagePath)
        ...
        .blur(2.5)     // smooth pixelation edges
        .toBuffer();
...
async function blurAllFaces() {
  ...
  for (const album of albums) {
    ...
    for (const file of files) {
      ...
      if (faces === null) {
        // Python error on this file: mark it to avoid retrying
  ...
  if (scanned > 0 || blurred > 0) {
    console.log(`  ✓ ${scanned} photo${scanned > 1 ? 's' : ''} scanned, ${blurred} face${blurred > 1 ? 's' : ''} blurred.`);
  }
}
