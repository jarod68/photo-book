 ...
 *
 * Détection : Python + OpenCV Haar cascades (frontal + profil).
 * Floutage   : Sharp — pixelisation (↓10 % nearest-neighbor → ↑ taille originale)
 *              + léger Gaussian blur aux bords pour un rendu propre.
 * Tracking   : <PHOTOS_DIR>/.faces-blurred.json — les fichiers déjà traités
 *              ne sont jamais re-analysés, même après redémarrage.
 * Originaux  : modifiés en place (via fichier .tmp pour l'atomicité).
 *              Les previews correspondants sont supprimés afin d'être
 *              régénérés par ensurePreview() au prochain accès.
 */
...
function detectFaces(imagePath) {
  return new Promise(resolve => {
    ...
    proc.on('error', () => resolve(null)); // python3 non trouvé
  });
}
...
async function blurFaces(imagePath, faces) {
  ...
  const composites = (
    await Promise.all(faces.map(async face => {
      ...
      // Pixelisation : descend à ~8 % de la taille puis remonte
      ...
      const blurred = await sharp(imagePath)
        ...
        .blur(2.5)     // lisse les bords de pixelisation
        .toBuffer();
...
// ── Supprime le preview pour forcer la régénération ───────────────────────────
...
async function blurAllFaces() {
  ...
  if (!available) {
    console.log('  ⚠  python3 introuvable — floutage des visages désactivé');
    console.log('     (Docker : assurez-vous que py3-opencv est installé)');
    return;
  }
  ...
  for (const album of albums) {
    ...
    for (const file of files) {
      ...
      if (tracker[key]) continue; // déjà traité

      process.stdout.write(`  ⟳  Analyse visages : ${key}\r`);
  ...
  const faceWord  = blurred > 1 ? 'visages floutés' : 'visage flouté';
  const photoWord = scanned > 1 ? 'photos analysées' : 'photo analysée';
